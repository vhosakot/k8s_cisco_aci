...
class Allocator:
    ...
    DEFAULT_VLAN_MIN = 2120
    DEFAULT_VLAN_MAX = 4000
    ...
    def __init__(self, etcd_client, **kwargs):
        ...
        if self.VLAN_MAX > 4094:
            raise ValueError("vlan_max must be <= 4094 (got %d)" % self.VLAN_MAX)

        # a reservation requires 2 vlan ids minimum
        if self.VLAN_MIN + 1 > self.VLAN_MAX:
            raise ValueError(
                "vlan_max must be at least 1 id higher than vlan_min (got %d and %d respectively)" % (self.VLAN_MIN, self.VLAN_MAX)
            )

        self.MAX_VLANS = self.VLAN_MAX - self.VLAN_MIN

        self.MULTICAST_RANGE = kwargs.get("multicast_range", self.DEFAULT_MULTICAST_RANGE)
        self.SERVICE_SUBNET = kwargs.get("service_subnet", self.DEFAULT_SERVICE_SUBNET)
    ...
    def __locked_reserve(self, tenant_name):
        ...
        if tenant_name == '' or ' ' in tenant_name:
            raise InvalidNameError("name must be one or more characters without spaces")

        # 1. load state from db
        state = self.load_from_db()

        if tenant_name in state:
            raise TenantAlreadyExistsError("tenant " + tenant_name + " already exists")
        ...
        unused_vlan_ids = set(range(self.VLAN_MIN, self.VLAN_MAX + 1)) - set(existing_vlan_ids)
        unused_vlan_ids = sorted(list(unused_vlan_ids))

        if len(unused_vlan_ids) < 2:
            raise InsufficientVLANsAvailableError(
                "unable to allocate 2 vlan ids, only %d ids available" % len(unused_vlan_ids)
            )
        ...
        if not svc_subnet_found:
            raise NoServiceSubnetsAvailableError(
                "unable to find a free service subnet, %d are already allocated" % len(existing_svc_subnets)
            )
        ...
        if not mcast_range_found:
            raise NoMulticastRangesAvailableError(
                "unable to find a free multicast range, %d are already allocated" % len(existing_mcast_ranges)
            )

        mcast_range_start, mcast_range_end = start_and_end_addresses_for_mcast_range(mcast_range)

        # 5. update the state object, convert to json, store in db
        state[tenant_name] = {
    ...
    def __locked_free(self, tenant_name):
        ...
        if tenant_name in state:
            ...
            raise TenantDoesNotExistError("tenant " + tenant_name + " does not exist")
